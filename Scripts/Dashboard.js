...
function makeGridster(div,readonly) {
    div.gridster({
        ...
        disableCollsionGrid: !readonly,
        min_cols: (readonly ? 1 : undefined),
        max_cols: (readonly ? MaxCols : undefined),
        min_rows: 1,
        max_rows: 500,
        avoid_overlapped_widgets: !isDesignerMode,
        shift_widgets_up: false,
        shift_larger_widgets_down: false,
        resize: {
            enabled: !readonly,
            ...
            stop: function (e, ui, wdg) {
                ...
                var info = WidgetSettingInfo[wdg.attr("id")];
                if (info.OnResize) {
                    for (var i = 0; i < info.OnResize.length; i++) {
                        if (typeof info.OnResize[i] == "function")
                            info.OnResize[i](info);                                
                    }
                }
            }
        },
...
function generateWidgetStyleTags() {
    ...
    for (var i = 100; i >= 1; i--) {
        styles += (ns + ' [data-col="' + (i) + '"] { left:' +
            ((i-1) * MinWidgetWidth +
            (i * margin))+'px; }\n');
    }

    for (var i = 500; i >= 1; i--) {
        styles += (ns + ' [data-row="' + (i) + '"] { top:' +
            ((i-1) * MinWidgetHeight +
            (i * margin) )+ 'px; }\n');
    }
...
function Wdg_Settings(a) {
    ...
    if (!divSetting.exists()) {
        ...
        divSetting = $("<div id='s_" + wdg.attr("id") + "' class='settingPanel GreyTheme'><div class='title'>Widget Settings</div><div class='_defaultSetting'><span class='row'><span class='lbl'>Choose Theme</span><span class='theme'>" + thm.join('') + "</span></span><span class='row autoRefresh'><span class='lbl'>Auto Refresh</span><input type='checkbox'/><input class='txt' style='display:none' type='text'><span style='display:none;vertical-align:middle;font-size: 12px;'> min</span></span></div><div class='content'></div></div>");
        divSetting.append("<div class='cmd'><a class='mdl-button RedButton' style='float:left' onclick='hideWidgetSetting($(\"#"+wdg.attr("id")+"\"))' href='javascript:void(0)'>Cancel</a><a class='mdl-button BlueButton _preview' onclick='previewWidgetSetting(this)' href='javascript:void(0)'>Preview</a><a onclick='saveWidgetSetting(this)' class='mdl-button GreenButton' href='javascript:void(0)'>Save</a></div>")
        if (Erp.Responsive)
            $("body").append(divSetting)
        else
    ...
    if (!Erp.Responsive) {
        ...
        divSetting.show().position({ my: "middle top+" + $(document).scrollTop(), at: "middle bottom", of: "body", collision: "fit none" })
...
function hideWidgetSetting(wdg) {
    $("#spnCallout").css("z-index", ""); $("#spnCallout,#divBg").hide(); wdg.removeClass("showSetting"); $("#s_" + wdg.attr("id")).hide();
...
function AddWidgetToDashBoard(d, info, delayLoad, readOnly) {
    ...
    //y=(px+25)/45  ~  20y+ 25y -25=px   ~     y *MinWidgetHeight  +  (y - 1) * WidgetGap/2=65px for y=2,MinWidgetHeight=20 and WidgetGap=50
    if (info.DefSizeX > 0)
        info.SizeX = Math.ceil((info.DefSizeX + 25) / 45);
    if (info.DefSizeY > 0)
        info.SizeY = Math.ceil((info.DefSizeY + 25) / 45);
    ...
    if (info.Max / 1 == 1) {
        ...
        var wx = isDesignerMode ? 26 : (readOnlyMode ? 12 : Math.ceil(((window.innerWidth - 50) + 25) / 45));
        var wy = isDesignerMode ? 18 : (Math.ceil(((window.innerHeight - 100) + 25) / 45));
        info.SizeX = info.SizeX1 = wx;
        info.SizeY = info.SizeY1 = wy;

        wx = isDesignerMode ? 21 : (readOnlyMode ? 12 : Math.ceil(((window.innerWidth - 50) + 25) / 45));
        wy = isDesignerMode ? 18 : (Math.ceil(((window.innerHeight - 70) + 12.5) / 32.5));
        info.SizeX2 = wx;
        info.SizeY2 = wy;

        wx = isDesignerMode ? 10 : (readOnlyMode ? 12 : Math.ceil(((window.innerWidth - 50) + 25) / 45));
        wy = isDesignerMode ? 18 : (Math.ceil(((window.innerHeight - 50) + 7.5) / 27.5));
        info.SizeX3 = wx;
        info.SizeY3 = wy;
    }
    var wdgUrl = "";
    ...
    if (!$.isEmpty(url)) {
        ...
        //info.Settings is stored in w.data("Settings")
    }
    ...
    '<div class="wdg ' + (readOnly ? "readOnly " : "") + (info.NoConstraint > 0 ? "noconstraint" + " " : "") + (info.NotResizable == true ? "noresize" + " " : "") + (info.Trans > 0 ? "trans" + info.Trans + " " : "") + (Erp.Responsive ? "" : (($.isEmpty(info.Settings.Theme) ? DefaultTheme : info.Settings.Theme) + "Theme")) + '" id="wdg_' + info.Uid + info.DashId + '" uid="' + $.defaultVal(info.Uid, "") + '" widgetid=' + info.Id + ' ' + (info.Col > -1 ? 'data-col="' + info.Col + '" data-row="' + info.Row + '" ' : '') + (d == null ? ' data-sizex="' + info.SizeX + '" data-sizey="' + info.SizeY + '"' : '') + '>' +
        (isDesignerMode ? "<a class='wdgdel' title='Remove Widget' href='javascript:void(0)' onclick='Wdg_Close(this)'></a>" : "") +
        '<div class="wdg-ctr">' +
            '<div class="wdg-top">' +
                ...
                '<div class="wdg-cmd">' +
                    ...
                    (readOnly || isDesignerMode ? '' : '<a title="Close Widget" onclick="Wdg_Close(this)" href="javascript:void(0)" class="wclose  mdl-ripple2"></a>') +
                '</div>' +
            '</div>' +
            '<iframe ' + (delayLoad ? '' : 'src="' + wdgUrl + '"') + ' class="' + (info.Trans > 1 ? "trans2" : "") + '" ' + (Modernizr.isios ? " scrolling=\"no\" " : "") + ' allowTransparency="allowTransparency"  frameborder="0" style="height: 100%;width: 1px; min-width: 100%' + ($.isEmpty(wdgUrl)?";display:none":"") + '"></iframe>' +
        '</div>' +
    ...
    if (readOnly || isDesignerMode) {
        if (info.SizeX1 > 0) {
            w.attr("dev-sizex1", info.SizeX1); w.attr("dev-sizey1", info.SizeY1);
        }
        if (info.Col1 > 0) {
            w.attr("dev-col1", info.Col1); w.attr("dev-row1", info.Row1); 
        }

        if (info.SizeX2 > 0) {
            w.attr("dev-sizex2", info.SizeX2); w.attr("dev-sizey2", info.SizeY2);
        }
        if (info.Col2 > 0) {
            w.attr("dev-col2", info.Col2); w.attr("dev-row2", info.Row2); 
        }

        if (info.SizeX3 > 0) {
            w.attr("dev-sizex3", info.SizeX3); w.attr("dev-sizey3", info.SizeY3);
        }
        if (info.Col3 > 0) {
            w.attr("dev-col3", info.Col3); w.attr("dev-row3", info.Row3); 
        }
    }
    var ui = w.children("._setting").find(".wdg-content");
    if (ui.length > 0)
        w.children(".wdg-ctr").append(ui);
    return w;
}
...
function saveWidgetSetting(a) {
    a = $(a);
    var wdg = $("#" + a.closest(".settingPanel").attr("id").substring(2));
    ...
    if (info.OnSaveSuccess) {
        for (var i = 0; i < info.OnSaveSuccess.length; i++) {
            if (typeof info.OnSaveSuccess[i] == "function")
                info.OnSaveSuccess[i](info, settings);
        }
    }
...
function _getNewSettings(wdg, pnl) {
    ...
    var prm = readCustomSettings(wdg.ID());
    if (prm === false)
        return false;
...
function resizeMaxWidgets() {
    ...
    var wx = readOnlyMode ? 12 : Math.ceil((window.innerWidth - 50 + 25) / 45);
    var wy = Math.ceil((window.innerHeight - 100 + 25) / 45);

    if (window.innerWidth < 600) {
        wx = readOnlyMode ? 12 : Math.ceil((window.innerWidth - 50 + 25) / 45);
        wy = Math.ceil((window.innerHeight - 50 + 7.5) / 27.5);
    }
    else if (window.innerWidth < 1010) {
        wx = readOnlyMode ? 12 : Math.ceil((window.innerWidth - 50 + 25) / 45);
        wy = Math.ceil((window.innerHeight - 70 + 12.5) / 32.5);
    }
    ...
    $("#dashBoardCtr>div>.wdg").each(function () {
        ...
        if (wdg.data("MaxSize")) {
            ...
            wdg.attr("dev-sizex" + m, wx);
            wdg.attr("dev-sizey" + m,wy);

            var g = wdg.closest(".gridster").data("gridster");
            g.resize_widget(wdg, wx, wy);
        }
    })
}
...
function switchView(mode,init) {    
    ...
    if (isDesignerMode) {
        ...
        $("#dashBoardCtr").removeClass("vw vw1 vw2 vw3").addClass("vw vw"+mode);
    }
    ...
    $("#dashBoardCtr").children(".gridster").each(function () {
        $(this).children(".wdg").each(function () {
            ...
            if (!w.attr("dev-col" + mode)) {
                w.attr("dev-col" + mode, w.attr("dev-col1")); w.attr("dev-row" + mode, w.attr("dev-row1"));
            }
            if (!w.attr("dev-sizex" + mode)) {
                 w.attr("dev-sizex" + mode, w.attr("dev-sizex1")); w.attr("dev-sizey" + mode, w.attr("dev-sizey1"));
            }
...
function setApplicableView(init) {
    ...
    var hasWidthIssue = false;
    $("#dashBoardCtr").children(".gridster").each(function () {
        $(this).children(".wdg").each(function () {
            if ($(this).outerWidth() < 30)
                hasWidthIssue = true;
        })
    });
    if (window.__currentViewMode != m || hasWidthIssue) {
