 ...
 ***DONT UPDATE*****
 Modified DATEPICKERS
 Modified Formselect _appendOptionWithIcon method to hide options
 modified floatingactionbutton hoverEnabled checking if touch screen now
 dropdown _setupTemporaryEventHandlers modified body to documentelement
 */
    ...
    var Dropdown = function (_Component2) {
        ...
        _createClass(Dropdown, [{
            ...
            value: function _setupTemporaryEventHandlers() {
                ...
                document.documentElement.addEventListener('click', this._handleDocumentClickBound, true);
                document.documentElement.addEventListener('touchend', this._handleDocumentClickBound);
                document.documentElement.addEventListener('touchmove', this._handleDocumentTouchmoveBound);
            ...
            value: function _removeTemporaryEventHandlers() {
                // Use capture phase event handler to prevent click
                document.documentElement.removeEventListener('click', this._handleDocumentClickBound, true);
                document.documentElement.removeEventListener('touchend', this._handleDocumentClickBound);
                document.documentElement.removeEventListener('touchmove', this._handleDocumentTouchmoveBound);
            ...
            value: function _animateIn() {
                ...
                anim({
                    ...
                    complete: function (anim) {
                        ...
                        var s = $(_this10.dropdownEl).children(".selected")[0];

                        if (s && $(s).parent()[0].scrollHeight > $(s).parent().height()) s.scrollIntoView({ block: 'nearest', inline: 'start' })
                        // onOpenEnd callback
...
; (function ($, anim) {
    ...
    var Modal = function (_Component3) {
        ...
        function Modal(el, options) {
            ...
            _this12.$overlay = $('<div class="modal-overlay ' + (_this12.options.cssClass ? _this12.options.cssClass : "") + '"></div>');
        ...
        _createClass(Modal, [{
            ...
            value: function open($trigger) {
                ...
                if (this.options.zIndex) {
                    this.$overlay[0].style.zIndex = this.options.zIndex;
                    this.el.style.zIndex = this.options.zIndex+1;
                }
    ...
    var Sidenav = function (_Component8) {
        ...
        _createClass(Sidenav, [{
            ...
            value: function _isCurrentlyFixed() {
                return this.isFixed && window.innerWidth > 1500;
            }
        }, {
...
; (function ($) {
    ...
    var Autocomplete = function (_Component10) {
        ...
        function Autocomplete(el, options) {
            ...
            if ($.isArray(options.data)) {
                ...
                for (var i = 0; i < options.data.length; i++)
                    if (typeof options.data[i] == "object" && options.data[i].hasOwnProperty("label")) {
                        options.isLabelValue = true;
                        d[options.data[i].label] = options.data[i].value;
                    }
                    else
                        d[options.data[i]] = null;
                options.data = d;
            }
        ...
        _createClass(Autocomplete, [{
            ...
            value: function _setupEventHandlers() {
                ...
                this.el.addEventListener('click', this._handleInputKeyupAndFocusBound);
            ...
            value: function _handleInputKeyupAndFocus(e) {
                ...
                if (this.oldVal !== val || this.options.showAllItems) {
                    this._resetAutocomplete();

                    if (this.options.showAllItems) {
                        this.isOpen = true;
                        this._renderDropdown(this.options.data, true);
                    }
            ...
            value: function selectOption(el) {
                if (!el || !el[0])
                    return;
                var text = (el.attr("val") ? el.attr("val") : el.text().trim());
                this.el.value = text;
                this.$el.trigger('change');
                this._resetAutocomplete();
                this.dropdown.close();
                window.setTimeout(function () { document.activeElement.blur(); }, 100);
            ...
            value: function _renderDropdown(data, val) {
                ...
                for (var key in data) {
                    if (data.hasOwnProperty(key) && (val===true || key.toLowerCase().indexOf(val) !== -1)) {
                        // Break if past limit
                ...
                for (var i = 0; i < matchingData.length; i++) {
                    ...
                    var $autocompleteOption = $('<li' + (this.options.isLabelValue ? ' val="' + _entry.data + '" ' : '') + '></li>');
                    if (!!_entry.data && !this.options.isLabelValue) {
    ...
    if (M.jQueryLoaded) {
        M.initializeJqueryWrapper(Autocomplete, 'autocompletex', 'M_Autocomplete');
    }
})(cash);
; (function ($) {
    ...
    M.textareaAutoResize = function ($textarea) {
        ...
        else if ($textarea.data('original-height') <= hiddenDiv.innerHeight()) {
    ...
    $(document).ready(function () {
        ...
        document.addEventListener('focus', function (e) {
            if ($(e.target).is(input_selector)) {
                if ($(e.target).hasClass("select-dropdown"))
                    $(e.target).closest(".erp-Select").find("._lbl").addClass('active');
            ...
            if ($inputElement.is(input_selector)) {
                ...
                if ($(e.target).hasClass("select-dropdown")) {
                    if ($inputElement[0].value.length === 0)
                        $(e.target).closest(".erp-Select").find("._lbl").removeClass('active');
                }
                else
...
; (function ($, anim) {
    ...
    var _defaults = {
        direction: 'top',
        hoverEnabled: !$(document.documentElement).hasClass("touch"),
...
; (function ($) {
    ...
    var Datepicker = function (_Component15) {
        ...
        function Datepicker(el, options) {
            ...
            if (!_this50.options.defaultDate) {
                var m = (_this50.el.value ? moment(_this50.el.value, _this50.options.format) : null);
                _this50.options.defaultDate = (m ? m.toDate() : null);// new Date(Date.parse(_this50.el.value));
                this.time = (m ? m.format(_this50.options.timeFormat) : "");
        ...
        _createClass(Datepicker, [{
            ...
            value: function _setupModal() {
                ...
                this.modal = M.Modal.init(this.modalEl, {
                    cssClass: "modal-date",
                    zIndex: _this51.options.zIndex?_this51.options.zIndex:1010,
                    onCloseEnd: function () {
                        
                        _this51.isOpen = false;
                        if (typeof _this51.options.onModalClose === 'function') {
                            _this51.options.onModalClose.call(_this51);
                        }
                    }
                });
            }
        },
        {
            ...
            value: function getDate() {
                if (!this.el.value)
                    return "";
                return moment(this.el.value, this.options.format).toDate();
            }
        },
        {
            key: "toString",
            ...
            key: "setInputDate",
            value: function setInputDate(v) {
                this.date = v;
                this.time = moment(v).format(this.options.timeFormat);
                this.el.value = moment(v).format(this.options.format);
            }
        }
        , {
            key: "setInputValue",
            ...
            value: function draw(force) {
                ...
                M.FormSelect.init(yearSelect, { classes: 'select-year' });
                M.FormSelect.init(monthSelect, { classes: 'select-month' });
            ...
            value: function _setupEventHandlers() {
                ...
                if (this.options.enableButton) {
                   $(this.el).next()[0].addEventListener('click', this._handleInputClickBound);
                    //this.el.addEventListener('keydown', this._handleInputKeydownBound);
                }
                else {
            ...
            value: function _handleTodayClick() {
                this.date = new Date();
                if (typeof this.options.onFinalize === 'function') {
                    this.options.onFinalize.call(this, this.date);
                }
                this.time = moment(this.date).format(this.options.timeFormat);
                this.gotoDate(this.date)
                this.setInputValue();
                this.close();
            }
            ...
            value: function _handleClearClick() {
                if (typeof this.options.onFinalize === 'function') {
                    this.options.onFinalize.call(this, null);
                }
                this.date = null;
                this.time = "";
                this.setInputValue();
            ...
            value: function _handleInputChange(e) {
                ...
                if (this.options.parse) {
                    date = moment(this.el.value, this.options.format).toDate();// this.options.parse(this.el.value, this.options.format);
                } else {
                    date = moment(this.el.value, this.options.format).toDate();
                }
            ...
            value: function _finishSelection() {
                ...
                if (this.el.hasTime) {
                    var t = this.$el.next();
                    t.val(this.time);
                    var tm = M.Timepicker.getInstance(t[0]);
                    tm.open()
                } else {
                    if (typeof this.options.onFinalize === 'function') {
                        this.options.onFinalize.call(this, this.date);
            ...
            key: "open",
            value: function open() {
                var refWin = __getMainWin(this.options);
                if (refWin != null) {
                    refWin.__refdatePicker = this;
                    var id = this.options.isDateTime ? "__datetimeHelper" : "__dateHelper";
                    
                    if (!refWin.$("#"+id).exists()) {
                        refWin.$(refWin.document.body).append("<input id='"+id+"' style='display:none' type='text'>");
                        refWin.$("#" + id)[this.options.isDateTime ? "materialDateTimepicker" : "materialDatepicker"]({
                            container: 'body', yearRange: 25, format: this.options.format, dateFormat: this.options.dateFormat, timeFormat: this.options.timeFormat,zIndex:3600, onModalClose: function () {
                                delete this.el.ownerDocument.defaultView.__refdatePicker;
                            }, onFinalize: function (a) {
                                var p = this.el.ownerDocument.defaultView.__refdatePicker;
                                if (p.options.isDateTime) {
                                    this.el.ownerDocument.defaultView.$(p.el).materialDateTimepicker("setDateTime", a);
                                }
                                else {
                                    p.setDate(a, true); p.setInputDate(a);
                                }
                                $(p.el).trigger("change")
                                var lbl = $(p.el).parent().find("label")
                                lbl.removeClass("active").addClass(!a ? "" : "active");
                            }
                        });
                    }
                    var inst = M.Datepicker.getInstance(refWin.$("#" + id)[0]);
                    inst.open();
                    inst.setDate(this.date);
                    return this;
                }
    ...
    if (M.jQueryLoaded) {
        M.initializeJqueryWrapper(Datepicker, 'materialDatepicker', 'M_Datepicker');
    }
})(cash);
; (function ($) {
    'use strict';
    ...
    var Timepicker = function (_Component16) {
        ...
        _createClass(Timepicker, [{
            ...
            value: function _updateTimeFromInput() {
                var value = (this.el.value ? moment(this.el.value, this.options.format).toDate() : null);
                this.amOrPm = (value ? (value.getHours() >= 12 ? "PM" : "AM") : "");
                this.hours = (value ? value.getHours() : 0);
                this.minutes = (value ? value.getMinutes() : 0);
            ...
            value: function getTime() {
                return (this.el.value ? moment(this.el.value, this.options.format).toDate() : null);
            }
        }, {
            key: "setTime",
        ...
        },
         {
             ...
             value: function open() {
                 ...
                 if (refWin != null) {
                     refWin.__refdatePicker = this;
                     if (!refWin.$("#__timeHelper").exists()) {
                         refWin.$(refWin.document.body).append("<input id='__timeHelper' style='display:none' type='text'>");
                         refWin.$("#__timeHelper").materialTimepicker({
                             container: 'body',zIndex:3600, date: false, twelveHour: true, format: this.options.format, onModalClose: function () { delete this.el.ownerDocument.defaultView.__refdatePicker; }, onFinalize: function (a) {
                                 var p = this.el.ownerDocument.defaultView.__refdatePicker;
                                 p.setTime(a);
                                 if ($(p.el).next().attr("for") == p.el.id)
                                     $(p.el).next().removeClass("active").addClass(!a ? "" : "active");
                             }
                         });
                     }
                     var inst = M.Timepicker.getInstance(refWin.$("#__timeHelper")[0]);
                     inst.setTime(this.el.value);
                     inst.open();
                     return;
                 }
             ...
             value: function done(e, clearValue) {
                 ...
                 this.el.focus();
                 if (this.el.hasDate) {
                     var dt = $(this.el).parent().find(".entity-date");
                     var t = this.el.value;
                     var d = moment(dt[0].M_Datepicker.date).format("DD/MM/YYYY");
                     dt[0].M_Datepicker.date = moment(d + " " + t, "DD/MM/YYYY " + this.options.format).toDate();
                     dt[0].M_Datepicker.time = t;
                     dt[0].M_Datepicker.setInputDate(dt[0].M_Datepicker.date);
                     dt[0].focus();
                     if (typeof this.options.onFinalize === 'function') {
                         this.options.onFinalize.call(this, dt[0].M_Datepicker.date);
                     }
                     dt.trigger('change');
                 }
                 else {
                     if (typeof this.options.onFinalize === 'function') {
                         this.options.onFinalize.call(this, this.el.value);
                     }
                 }
    ...
    if (M.jQueryLoaded) {
        M.initializeJqueryWrapper(Timepicker, 'materialTimepicker', 'M_Timepicker');
    }
})(cash);
; (function ($) {
    'use strict';
...
; (function ($) {
    ...
    var FormSelect = function (_Component20) {
        ...
        function FormSelect(el, options) {
            ...
            _this67.isExpanded = _this67.$el.attr('expanded') == "1";
        ...
        _createClass(FormSelect, [{
            ...
            value: function _setupEventHandlers() {
                ...
                $(this.dropdownOptions).find('li:not(.optgroup)').each(function (el) {
                    if (Modernizr.touch)
                        el.addEventListener("touchmove", _this68._handleOptionClickBound);
                    el.addEventListener(Modernizr.touch?"touchend":'click', _this68._handleOptionClickBound);
                });

                //$(this.dropdownOptions).on("click", 'li:not(.optgroup)', _this68._handleOptionClickBound);
               ...
               this.dropdown.close()
                ...
                if (!$(option).hasClass('disabled') && !$(option).hasClass('optgroup') && key.length) {
                    ...
                    if (this.isMultiple) {
                        ...
                        //$(option).removeClass("selected").addClass(checkbox.prop('checked') ? "selected" : "")
                        selected = this._toggleEntryFromArray(key);
                    }
                    else if (this.isExpanded) {
                       
                        $(this.dropdownOptions).find('li').removeClass('active');
                        $(this.dropdownOptions).find('input[type="radio"]').prop('checked', false);
                        var checkbox = $(option).find('input[type="radio"]');
            ...
            value: function _setupDropdown() {
                ...
                this.wrapper = document.createElement('div');
                $(this.wrapper).addClass('select-wrapper' + ' ' + this.options.classes + (this.isExpanded ? ' select-expanded' : ''));
                this.$el.before($(this.wrapper));
                ...
                if (!this.el.disabled) {
                    ...
                    if (this.isMultiple || this.isExpanded) {
            ...
            value: function _appendOptionWithIcon(select, option, type) {
                ...
                var multipleCheckbox = this.isMultiple || this.isExpanded ? "<label><input class='with-gap' type=\"" + (this.isMultiple ? "checkbox" : "radio") + "\"" + disabledClass + "\"/><span>" + option.text + "</span></label>" : option.text;
                var liEl = $('<li style="' + (option.hidden ? "display:none" : "") + '" ></li>');
            ...
            value: function _setValueToInput() {
                ...
                options.each(function (el) {
                    if ($(el).prop('selected')) {
                        var text = $(el).text().indexOf("<")>-1?$("span").html($(el).text()).text():$(el).text();
                ...
                $(this.input).closest(".erp-Field").find("._lbl").removeClass("active").addClass(value === "" ? "" : "active");
            ...
            value: function _setSelectedStates() {
                ...
                for (var key in this._valueDict) {
                    ...
                    if ($(option.el).prop('selected')) {
                        $(option.optionEl).find('input[type="checkbox"],input[type="radio"]').prop("checked", true);
                        this._activateOption($(this.dropdownOptions), $(option.optionEl));
                        this._keysSelected[key] = true;
                    } else {
                        $(option.optionEl).find('input[type="checkbox"],input[type="radio"]').prop("checked", false);
            ...
            value: function _activateOption(collection, newOption) {
                if (newOption) {
                    ...
                    if (this.isMultiple || this.isExpanded) {
                        collection.find('li').each(function () { console.log(); $(this).removeClass('selected active'); if ($(this).find("input").prop("checked")) $(this).addClass('selected'); })
                    }
                    else {
...
$.fn.simpleTime = function (opt) {
    return this.each(function () {
        ...
        if ($(this).attr("placeholder")) {
            ...
            $(this).on("focus", function () { $(this).attr("placeholder", $(this).attr("_placeholder")); })
            $(this).on("blur", function () { $(this).removeAttr("placeholder"); if ($(this).val() == "") $(this).parent().children("._lbl").removeClass("active"); })
        }
        $(this).timepicker(opt);

    })
};
$.fn.simpleDate = function (opt) {
    return this.each(function () {
        if (!$(this).exists())
            return;
        ...
        for (var i = 0; i < f.length; i++) {
            if (f[i].toLowerCase().indexOf("d") > -1)
                dp.push('d');
            else if (f[i].toLowerCase().indexOf("m") > -1)
                dp.push('m');
            else if (f[i].toLowerCase().indexOf("yyyy") > -1)
                dp.push('Y');
            else if (f[i].toLowerCase().indexOf("y") > -1)
                dp.push('y');
        }
        if (dp.length > 2) {
            $(this).data("_cleave", new Cleave(this, {
                date: true,
                datePattern: dp,
                delimiter: sep,
            }));
            opt.enableButton = true;
            $(this).after("<a class='_dtp'></a>");
        }
        else
            $(this).attr("readonly","readonly");
        
       
        $(this).materialDatepicker(opt);
    })
};


/////-------DateTimepicker--------//////
$.fn.materialDateTimepicker = function (options, val) {
    var element = $(this);
    if (options === "destroy") {
        element.materialDatepicker("destroy");
        (element.next()[0] ? element.next().remove() : "");
        return element;
    }
    else if (options === "getDateTime") {
        var ref = (element.next().hasClass("ui-datepicker-trigger") ? element.next() : element);
        var d = element.materialDatepicker("getDate");
        var t = ref.parent().find(".entity-time").materialTimepicker('getTime');
        if (d == null || d=="") return null;
        t = (t == null ? "00:00 AM" : ref.parent().find(".entity-time").val());
        ...
        var ref = (element.next().hasClass("ui-datepicker-trigger") ? element.next() : element);
        element.materialDatepicker("setInputDate", val);
        if (val instanceof Date)
            ref.parent().find(".entity-time").materialTimepicker('setTime', val);
    ...
    element.addClass("entity-date");
    
    element[0].hasTime = true;
    element.materialDatepicker({ container: options.container, zIndex: options.zIndex, useParentPicker: options.useParentPicker, yearRange: 25, isDateTime: true, format: options.dateFormat + " " + options.timeFormat, dateFormat: options.dateFormat, timeFormat: options.timeFormat, onFinalize: options.onFinalize });
    var inputTime = $("<input style='display:none' class='entityInput entity-time ui-text' id='" + element.attr("id") + "_time' type='text' />");
    ...
    inputTime[0].hasDate = true;
    inputTime.materialTimepicker({ container: options.container, zIndex: options.zIndex, useParentPicker: options.useParentPicker, format: options.timeFormat, onFinalize: options.onFinalize, onModalClose: options.onModalClose });

    if (element.val()) {
        var t = moment(element.val(), options.dateFormat + " " + options.timeFormat);
        inputTime.val(t.format(options.timeFormat));
    }
    if (!$.isEmpty(element.attr("placeholder"))) {
        inputTime.attr("placeholder", element.attr("placeholder"))
    }
}


function __getMainWin(opt) {
    if (opt && opt.useParentPicker) { }
    ...
    if (Erp.IsCrossOrigin())
        return null;
    if (window.frameElement && (window.innerHeight < 500 || (opt && opt.useParentPicker))) {
        if (opt && opt.useParentPicker)
            refWin = refWin.parent;

        while (refWin.innerHeight < 500 || !refWin.M || typeof refWin.M.Datepicker!="function"){
            
            if (refWin.innerHeight > 400 && refWin.M && typeof refWin.M.Datepicker == "function")
                nextAvl = refWin;

            refWin = refWin.parent;
            if (Erp.IsCrossOrigin(refWin))
                return null;
            if (refWin == refWin.parent)
                break;
        }
        if (refWin && refWin.M && typeof refWin.M.Datepicker == "function")
            return refWin;
        else if (nextAvl)
            return nextAvl;
        else
...
$.fn.swipeTab = function (options) {
    var element = $(this);
    //if (!Erp.Touch)
    //    return element.tabs();
    if (options == "refresh") {
        if (Erp.Touch)
            $(this).children(".tab-ctr").dragend();
        var p = (element.hasClass("header-tab") ? $("#header-navBar").children(".tabs") : element.children(".tabs"));
        var t = p.find(".active").parent(); 
        p.children(".indicator").css({ left: p.scrollLeft() + t.position().left, width: t.outerWidth() });
        __swipeTabClick(t);
    }
    else {
        if (Erp.Touch)
            $(this).children(".tab-ctr").dragend({
                onSwipeEnd: function (a, b, c) {
                    a = $(a)
                    if (a.hasClass("_clicked")) {
                        a.removeClass("_clicked");
                        return;
                    }
                    var p = (a.parent().hasClass("header-tab") ? $("#header-navBar").children(".tabs") : a.parent().children(".tabs"));
                    var t = p.find("a").removeClass("active").filter("[tab='" + $(b).attr("id") + "']").addClass("active");
                    p.scrollLeft(0);
                    p.scrollLeft(t.position().left);
                    p.children(".indicator").css({ left: p.scrollLeft() + t.position().left, width: t.outerWidth() });

                }
            });
        
        var tabs = (element.hasClass("header-tab") ? $("#header-navBar").children(".tabs") : element.children(".tabs"));
        tabs.on("click", ".tab", function () {
            if ($(this).hasClass("disabled"))
                return;
            __swipeTabClick($(this));
        });
        var t = tabs.find(".active").eq(0).parent();
        if (t.length == 0)
            t = tabs.children(".tab:not(.disabled)").eq(0);
        if (t.length > 0) {
            __swipeTabClick(t, true);
            tabs.scrollLeft(t.position().left);
        }
    }
}

function __swipeTabClick(el, onload) {    
    el.parent().find("a").removeClass("active");
    el.find("a").addClass("active");
    var arr = el.parent().children(".tab:not(.disabled)");
    var ind = $.inArray(el[0], arr);
    el.parent().children(".indicator").css({ left: el.parent().scrollLeft() + el.position().left, width: el.outerWidth() });
    var t = el.parent().parent().attr("id") == "header-navBar" ? $('#header-tabBar').children(".tabs-wrapper").eq(0).children(".tab-ctr") : el.parent().parent().children(".tab-ctr");
    ...
    if (onload) {
        if (Erp.Touch)
            ...
            t.children().hide().eq(el.index()).show();
    }
    else {
        if (Erp.Touch) {
            t.addClass("_clicked");
...
$.fn.splitButton = function () {
    var element = $(this);
    element.children(".erp-Button").children('.arrow')
        .on('click', function (e) {
            e.stopPropagation();
            $(this).closest(".btn-wrapper").addClass('active');
            $(this).closest(".btn-wrapper").children(".btnctr").focus().css("min-width", Math.max($(this).parent().outerWidth() + 10, 200));
            
        })
    element.children(".btnctr")
        .on('click', function (e) { e.stopPropagation(); })
        .on('focusout', function (e) { window.setTimeout(function (x) {if(x.closest(".btnctr")[0]!=document.activeElement) x.closest(".btn-wrapper").removeClass('active'); }, 50, $(this)); });

    element.children(".btnctr")
       .on('click', "label,input,select", function (e) { $(this).closest('.btnctr').focus(); })
}