<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DashBoard2.aspx.cs" Inherits="SensysErp.Main.DashBoard2" %>
...
<head runat="server">
    ...
    <%# Readonly?"":HelperLib.Web.WebResources.GetResource("~/css/WidgetMap.css")%>
    ...
    <%# HelperLib.Web.WebResources.GetResource("~/Scripts/dashboard2.js")%>
...
<body class="pg-dashboard">
    <form id="form1" runat="server">
       ...
       <div class="dashTitleBar">
        ...
        <span id="spnSaveUserLayout" style="display: none;">
          ...
          <asp:Literal runat="server" EnableViewState="false" Mode="PassThrough" ID="ScriptRender"></asp:Literal>
    </form>
    <style>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            for (var i = 0; i < DashboardData.length; i++) {                
                ...
                for (var x = 0; x < arr.length; x++) {
                    g.append(AddWidgetToDashBoard(null, arr[x], d.Default ? false : true,(readOnlyMode==1 || d.IsReadonly==1)));
                }
                if(readOnlyMode==1)
        ...
        function showWidgetList() {
            wdgPopup = $(document.body).ShowPopup({ url: "../meta/DashboardWidget_List.aspx?mode=" + (getQS("my") == "1" ? "" : "design"), popupClass: "dashlist-ctr", anim: false, height: 150, width: 150, reuse: true, title: "Add Widgets To Dashboard", });
            wdgPopup.css({ height: "", width: "", right: wdgPopup.offset().left, bottom: 400 }).animate({ top: (screen.height < 1024 ? 20 : 50), left: (screen.height < 1024 ? 15 : 125), bottom: (screen.height < 1024 ? 20 : 50), right: (screen.height < 1024 ? 15 : 125) });
    ...
    <script>
        ...
        function readCustomSettings(id) {
            var userdata = {};
            var pnlSetting = $("#" + id).find(".settingPanel>.content");
            var info = WidgetSettingInfo[id];
            var keys = info.Keys;
            if (info.OnSave) {
                Erp.OnSave.EventList = info.OnSave;
