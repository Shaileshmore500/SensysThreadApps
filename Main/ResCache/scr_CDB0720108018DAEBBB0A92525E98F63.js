...
function html_WorkExperience_Valid(elem,data,field){
  ...
  var months=0.0;
  var exp=0.0;
  var totalExp=Number(Erp.GetFieldValue("html_ToatalWorkExp")).toFixed(2);
  
  
  var limit=Erp.GetParam(window, "allowed_experience_month_boundaries");
  ...
  if(Fn.GetProp("mode") == "_EmpID"){
    ...
    var max_boundaries=(Math.floor(totalExp)*12)+Number(totalExp.split('.')[1])+limit;
    //max_boundaries=Number(max_boundaries+"."+(max_boundaries- Math.floor((max_boundaries/12))*12));
    
    var min_boundaries=(Math.floor(totalExp)*12)+Number(totalExp.split('.')[1])-limit;
   // min_boundaries=Number(min_boundaries+"."+(min_boundaries- Math.floor((min_boundaries/12))*12));
    
    if (!(exp <= Erp.GetFieldValue("html_ToatalWorkExp")+limit && exp >= Erp.GetFieldValue("html_ToatalWorkExp")-limit))
    {
