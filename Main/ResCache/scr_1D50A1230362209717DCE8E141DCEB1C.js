...
Erp.FieldInfo.push({Name:"tbl_va_employeeinformation_employeetitle",ID:"html_emptitle",Disabled:true,Type:"Text",DataType:"SINGLESELECT",CustomRender:false,Mandatory:true,IsDescriptive:false,ReqForCalc:false,Data:null});
Erp.FieldInfo.push({Name:"tbl_va_employeeinformation_firstname",ID:"html_Fname",Disabled:true,Type:"Text",DataType:"TEXT",CustomRender:false,Mandatory:true,IsDescriptive:false,ReqForCalc:true,Data:null});
...
Erp.FieldInfo.push({Name:"tbl_va_employeeinformation_gender",ID:"html_gender",Disabled:true,Type:"Text",DataType:"SINGLESELECT",CustomRender:false,Mandatory:true,IsDescriptive:false,ReqForCalc:false,Data:null});
...
Erp.FieldInfo.push({Name:"_sel_bloodgroup",ID:"sel_bloodgroup",Disabled:true,Type:"Text",DataType:"SINGLESELECT",CustomRender:false,Mandatory:false,IsDescriptive:false,ReqForCalc:false,Data:null});
$('#html_positionInfo').on('click',function(){onFieldChange(html_positionInfo_Click,$(this));});
...
Erp.FieldInfo.push({Name:"tbl_va_employeeinformation_bloodgroup",ID:"html_bloodgroup",Disabled:true,Type:"Text",DataType:"TEXT",CustomRender:false,Mandatory:false,IsDescriptive:false,ReqForCalc:false,Data:null,OnLoad:(typeof html_bloodgroup_Load=='function'?html_bloodgroup_Load:null)});
...
Erp.FieldInfo.push({Name:"tbl_va_employeeinformation_maritalstatus",ID:"html_maritalstatus",Disabled:true,Type:"Text",DataType:"SINGLESELECT",CustomRender:false,Mandatory:true,IsDescriptive:false,ReqForCalc:false,Data:null});
...
var ddl=el;ddl.parent().addClass('ui-list');ddl.data('InlineMode',true);ddl.data('FieldID','uid_9');ddl.data('LookupCode','');ddl.data('Eids','tbl_VA_relationmaster');ddl.data('FieldType','SINGLESELECT_TABLE');ddl.data('Multi',false);enableKeyboardForList(ddl);ddl.parent().on('click',function(){showSearchList($(this));});
...
var ddl=el;ddl.parent().addClass('ui-list');ddl.data('InlineMode',true);ddl.data('FieldID','uid_40');ddl.data('LookupCode','');ddl.data('Eids','tbl_VA_reasonmaster');ddl.data('FieldType','SINGLESELECT_TABLE');ddl.data('Multi',false);enableKeyboardForList(ddl);ddl.parent().on('click',function(){showSearchList($(this));});
...
var ddl=el;ddl.parent().addClass('ui-list');ddl.data('InlineMode',true);ddl.data('FieldID','uid_66');ddl.data('LookupCode','');ddl.data('Eids','tbl_VA_degreemaster');ddl.data('FieldType','SINGLESELECT_TABLE');ddl.data('Multi',false);enableKeyboardForList(ddl);ddl.parent().on('click',function(){showSearchList($(this));});
...
Before we get started we need to have your identity verified, please proceed to begin with your verification process by clicking the button below.`)
  ...
  if (Erp.LayoutMode == "A" && !Fn.IsEmpty(Fn.GetProp("tilesid")))
  {
    ...
    Erp.SetParam(window,"tilesid",Fn.GetProp("tilesid"));
...
function LoadCompleteFunction(){
  ...
  */
  ...
  if(Erp.LayoutMode!="A")
    $("#HeaderPanel").find("#header-page-title").append("<span><b>  (Temp ID :- "+Erp.GetFieldValue("html_applicationno")+")<b></span>")
 ...
 Erp.SetParam(window,"tilesid",Erp.GetFieldValue("html_position_fid"));
    Erp.LoadVariable('tbl_pos',function(){
      ...
      {
     Erp.SetLabel("html_monthlysal","Monthly Salary (Range: "+Fn.Round(tbl_pos[0]["ctcfromrange"]/12,2) +" - " +Fn.Round(tbl_pos[0]["ctctorange"]/12,2) +")"  );
       $("#html_monthlysal-Ctr")[0]["dataset"]["balloon"]="Monthly Salary (Range: "+Fn.Round(tbl_pos[0]["ctcfromrange"]/12,2) +" - " +Fn.Round(tbl_pos[0]["ctctorange"]/12,2) +")"; 
      }      
    }
                    );
  //if(Fn.GetProp("mode")=="_EmpID")
...
function html_WorkExperience_Valid(elem,data,field){
  ...
  var months=0.0;
  var exp=0.0;
  var totalExp= Math.floor(Erp.GetFieldValue("html_ToatalWorkExp"))*12;
  if(Erp.GetFieldValue("html_ToatalWorkExp")>Math.floor(Erp.GetFieldValue("html_ToatalWorkExp")))
    totalExp+=Number((''+Erp.GetFieldValue("html_ToatalWorkExp")).split('.')[1])
  
  
  
  var limit=Number(Erp.GetParam(window, "allowed_experience_month_boundaries"));
  //limit=Number(limit<10?'0.'+limit:limit);
  
  var prev = Erp.Repeater.GetTokens("rpt_prevemployerinfo");
  if(Fn.GetProp("mode") == "_EmpID"){
    for(var i = 0;i<prev.length;i++)
    {
      years+=Math.floor(Fn.CFlt(Erp.GetFieldValue("html_WorkExperience-"+prev[i].token)));
      //months+=Fn.CFlt(Erp.GetFieldValue("html_WorkExperience-"+prev[i].token))-Math.floor(Fn.CFlt(Erp.GetFieldValue("html_WorkExperience-"+prev[i].token)));
      
      if(Erp.GetFieldValue("html_WorkExperience-"+prev[i].token)>Math.floor(Erp.GetFieldValue("html_WorkExperience-"+prev[i].token)))
      	months+=Number((''+Erp.GetFieldValue("html_WorkExperience-"+prev[i].token)).split('.')[1]);
      
      //exp += Fn.CFlt(Erp.GetFieldValue("html_WorkExperience-"+prev[i].token));
    }
    
    exp= Number(years*12 + months);//(Math.floor(months/12))+'.'+(months-(Math.floor(months/12)*12)));
    
    
    
    var max_boundaries=totalExp+limit;//(Math.floor(totalExp)*12)+Number(totalExp.split('.')[1])+limit;
    //max_boundaries=Number(max_boundaries+"."+(max_boundaries- Math.floor((max_boundaries/12))*12));
    
    var min_boundaries=totalExp-limit;//(Math.floor(totalExp)*12)+Number(totalExp.split('.')[1])-limit;
   // min_boundaries=Number(min_boundaries+"."+(min_boundaries- Math.floor((min_boundaries/12))*12));
    
    if (!(exp <= max_boundaries && exp >= min_boundaries))
    {
